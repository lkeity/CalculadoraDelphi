...
begin
//Verifica se foi '='. Se houve, limpa a informação na tela.
  if achoures or (tresultado.Text = '0') then
    begin
      tresultado.Clear;
      tcalculo.Clear;
    end;
...
// Se tem informação na tela, valida.
...
// Se clicou em mais de um '=', verifica se há possibilidade de calcular a mesma coisa de novo.
  if achoures then
// executar a operação
    begin
// Se tem operação. Caso não tenha, ele apagará a tela no próximo '='
      if opa <> '' then
        begin
// Concatena os dados e joga na tela provisória.
...
// Se foi só uma vez, verifica se há operação pendente.
    begin
      RealizaOperacaoPendente;
    end;
...
// Se tem informação na tela, troca o sinal.
  if visorResultado <> '' then
    begin
      if sinal = '-' then
        begin
...
begin
  ...
  visorResultado := tresultado.Text;
// Se tem informação na tela, faz o cálculo da raiz quadrada.
  if visorResultado <> '' then
    begin
// Se foi a primeira ou segunda vez que tecla o botão de raiz quadrada.
...
begin
  ...
  visorResultado := tresultado.Text;
// Verifica se na tela foi solicitado a porcentagem antes ou depois da operação.
      if op = '' then
        begin
          tresultado.Text := '0';
        end
      ...
      if tresultado.Text <> '' then
        begin
          ...
          if (op = '/') and (num1 = 0) then
            begin
              tcalculo.Text := 'Não é possível dividir por zero';
              tresultado.Text := '';
              op := '';
              num1 := 0;
              num2 := 0;
...
//Se tem algo na tela e não foi digitado operação ainda.
    if tresultado.Text <> '' then
      begin
